                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input type="hidden" name="wholesale_product" value="0">
                                
                                <!-- Custom Toggle Switch -->
                                <div class="custom-toggle-wrapper" style="display: inline-block;">
                                    <input type="checkbox" id="wholesale_product_toggle" name="wholesale_product" value="1" {{ $product->wholesale_product ? 'checked' : '' }} style="display: none;">
                                    <label for="wholesale_product_toggle" class="custom-toggle-label" style="
                                        position: relative;
                                        display: inline-block;
                                        width: 50px;
                                        height: 25px;
                                        background: {{ $product->wholesale_product ? '#28a745' : '#ccc' }};
                                        border-radius: 25px;
                                        cursor: pointer;
                                        transition: background 0.3s;
                                    ">
                                        <span class="custom-toggle-slider" style="
                                            position: absolute;
                                            top: 2px;
                                            left: {{ $product->wholesale_product ? '27px' : '2px' }};
                                            width: 21px;
                                            height: 21px;
                                            background: white;
                                            border-radius: 50%;
                                    ...
                                    <span class="toggle-status ml-2" style="font-weight: bold; color: {{ $product->wholesale_product ? '#28a745' : '#dc3545' }};">
                                        {{ $product->wholesale_product ? 'Enabled' : 'Disabled' }}
                                    </span>
                                </div>
                            </div>
                        </div>
                        
                        <div id="wholesale-options" style="display: {{ $product->wholesale_product ? 'block' : 'none' }}; transition: all 0.3s ease;">
                            <div class="form-group row">
                                <label class="col-md-3 col-from-label">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input type="hidden" name="ecomarch_product" value="0">
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input type="checkbox" name="ecomarch_product" value="1" {{ $product->ecomarch_product ? 'checked' : '' }}>
                                    <span></span>
                                </label>
                                <small class="text-muted d-block mt-1">{{translate('Enable this to show the product in regular All Products section')}}</small>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function (){
        ...
        // Ensure wholesale section is shown/hidden correctly on page load
        setTimeout(function() {
            toggleWholesaleOptions();
        }, 100);

        // Listen for changes to the wholesale checkbox
        $(document).on('change', '#wholesale_product_toggle, input[name="wholesale_product"]', function() {
            console.log('Wholesale checkbox changed:', $(this).is(':checked'));
            toggleWholesaleOptions();
        });
    });
    ...
    function initCustomToggle() {
        var checkbox = document.getElementById('wholesale_product_toggle');
        var label = document.querySelector('.custom-toggle-label');
        var slider = document.querySelector('.custom-toggle-slider');
        var status = document.querySelector('.toggle-status');
        var wholesaleOptions = document.getElementById('wholesale-options');
        
        if (!checkbox || !label || !slider || !status || !wholesaleOptions) {
            return;
        }
