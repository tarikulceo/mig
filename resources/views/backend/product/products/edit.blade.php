                        ...
                        <div class="bg-white p-3 p-sm-2rem">
                            ...
                            <div class="w-100">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-6">
                                        <input type="number" lang="en" min="0" step="0.01" placeholder="{{ translate('Purchase Price') }}" name="purchase_price" class="form-control @error('purchase_price') is-invalid @enderror" value="{{$product->purchase_price ?? 0}}">
                                    </div>
                                </div>
                                ...
                                <!-- Sales Commission (Only for In-house Products) -->
                                @if($product->added_by == 'admin')
                                <div class="form-group row">
                                    <label class="col-md-3 col-from-label">
                                        {{translate('Sales Commission')}}
                                        <span class="text-info" data-toggle="tooltip" title="{{translate('Enable commission for sales representatives on this product. Only applies to in-house products.')}}">
                                            <i class="las la-info-circle"></i>
                                        </span>
                                    </label>
                                    <div class="col-md-9">
                                        <label class="aiz-switch aiz-switch-success mb-0">
                                            <input type="checkbox" name="enable_sales_commission" id="enable_sales_commission" value="1" {{ $product->enable_sales_commission ? 'checked' : '' }} onchange="toggleCommissionPercentage()">
                                            <span></span>
                                        </label>
                                        <small class="text-muted d-block">{{translate('Enable commission for sales representatives on this product')}}</small>
                                    </div>
                                </div>

                                <!-- Commission Percentage -->
                                <div class="form-group row" id="commission-percentage-row" style="display: {{ $product->enable_sales_commission ? 'flex' : 'none' }};">
                                    <label class="col-md-3 col-from-label">{{translate('Commission Percentage')}} <span class="text-danger">*</span></label>
                                    <div class="col-md-6">
                                        <div class="input-group">
                                            <input type="number" lang="en" min="0" max="100" value="{{ $product->sales_commission_percentage ?? 0 }}" step="0.01" placeholder="{{ translate('Enter percentage (e.g., 5.50)') }}" name="sales_commission_percentage" class="form-control @error('sales_commission_percentage') is-invalid @enderror" {{ $product->enable_sales_commission ? 'required' : '' }}>
                                        ...
                                        @enderror
                                        <small class="text-muted">{{translate('Percentage of the product selling price that will be paid as commission to sales representatives')}}</small>
                                    </div>
                                </div>
                                @endif
                                ...
                                <div id="show-hide-div">
                                    ...
                                    <div class="form-group row">
                                        <label class="col-md-3 col-from-label">{{translate('Warehouse Stock')}} <span class="text-danger">*</span></label>
                                        <div class="col-md-9">
                                            <div class="card">
                                                <div class="card-header">
                                                    <h6 class="mb-0">{{translate('Manage Stock by Warehouse')}}</h6>
                                                </div>
                                                <div class="card-body">
                                                    <div id="warehouse-stock-container">
                                                        ...
                                                        @if($warehouses->count() > 0)
                                                            ...
                                                            @php
                                                                // Get warehouse stock for this specific warehouse
                                                                $warehouseStocks = $product->warehouse_stocks->where('warehouse_id', $warehouse->id);
                                                                $stockQty = 0;
                                                                if ($warehouseStocks->count() > 0) {
                                                                    $stockQty = $warehouseStocks->sum('quantity');
                                                                }
                                                                $totalStock += $stockQty;
                                                            @endphp
                                                            <div class="row mb-3 warehouse-stock-row">
                                                                <div class="col-md-6">
                                                                    <label class="form-label">{{ $warehouse->name }}</label>
                                                                    <input type="hidden" name="warehouse_ids[]" value="{{ $warehouse->id }}">
                                                                </div>
                                                                <div class="col-md-6">
                                                                    <input type="number" min="0" value="{{ $stockQty }}" step="1" 
                                                                           placeholder="{{ translate('Stock Quantity') }}" 
                                                                           name="warehouse_stocks[{{ $warehouse->id }}]" 
                                                                           class="form-control warehouse-stock-input">
                                                                </div>
                                                            </div>
                                                            @endforeach
                                                            <div class="row mt-3">
                                                                <div class="col-md-12">
                                                                    <label class="form-label">{{translate('Total Stock')}}: <span id="total-stock">{{ $totalStock }}</span></label>
                                                                    <input type="hidden" name="current_stock" id="total-stock-input" value="{{ $totalStock }}">
                                                                </div>
                                                            </div>
                                                        @else
                                                            <div class="alert alert-warning">
                                                                <h6>{{translate('No Warehouses Available')}}</h6>
                                                                <p>{{translate('Please create at least one warehouse before managing stock.')}}</p>
                                                            ...
                                                            <div class="row">
                                                                <div class="col-md-6">
                                                                    <label class="form-label">{{translate('Current Stock')}}</label>
                                                                </div>
                                                                <div class="col-md-6">
                                                                    <input type="number" min="0" value="{{ $product->getTotalStock() }}" step="1" 
                                                                           placeholder="{{ translate('Stock Quantity') }}" 
                                                                           name="current_stock" 
                                                                           class="form-control">
                                                                </div>
                            ...
                            <!-- Wholesale Product -->
                            <h5 class="mb-3 pb-3 fs-17 fw-700" style="border-bottom: 1px dashed #e4e5eb;">{{translate('Wholesale Product')}}</h5>
                            <div class="w-100 mb-3">
                                <div class="form-group row">
                                    <label class="col-md-3 col-from-label">{{translate('Enable Wholesale Product')}}</label>
                                    <div class="col-md-9">
                                        <label class="aiz-switch aiz-switch-success mb-0">
                                            <input type="checkbox" name="wholesale_product" value="1" {{ $product->wholesale_product ? 'checked' : '' }} onchange="toggleWholesaleOptions()">
                                            <span></span>
                                        </label>
                                    </div>
                                </div>
                                
                                <div id="wholesale-options" style="display: {{ $product->wholesale_product ? 'block' : 'none' }};">
                                    <div class="form-group row">
                                        <label class="col-md-3 col-from-label">
                                        ...
                                        <div class="col-md-9">
                                            <div class="wholesale-price-section">
                                                @php
                                                    ...
                                                    if($product->wholesale_product && $product->stocks->isNotEmpty()) {
                                                        $wholesalePrices = collect($product->stocks->first()->wholesalePrices ?? []);
                                                    }
                                                @endphp
                                                
                                                @if($wholesalePrices->isNotEmpty())
                                                    @foreach($wholesalePrices as $wholesale)
                                                    <div class="row gutters-5 wholesale-price-row">
                                                        <div class="col-3">
                                                            <div class="form-group">
                                                                <input type="number" class="form-control" placeholder="{{translate('Min QTY')}}" name="wholesale_min_qty[]" value="{{ $wholesale->min_qty }}" min="1">
                                                            </div>
                                                        </div>
                                                        <div class="col-3">
                                                            <div class="form-group">
                                                                <input type="number" class="form-control" placeholder="{{ translate('Max QTY') }}" name="wholesale_max_qty[]" value="{{ $wholesale->max_qty }}" min="1">
                                                            </div>
                                                        </div>
                                                        <div class="col">
                                                            <div class="form-group">
                                                                <input type="number" step="0.01" class="form-control" placeholder="{{ translate('Price per piece') }}" name="wholesale_price[]" value="{{ $wholesale->price }}" min="0">
                                                            </div>
                                                        </div>
                                                        <div class="col-auto">
                                                            <button type="button" class="mt-1 btn btn-icon btn-circle btn-sm btn-soft-danger" onclick="removeWholesalePriceRow(this)">
                                                ...
                                                @else
                                                    <div class="row gutters-5 wholesale-price-row">
                                                        <div class="col-3">
                                                            <div class="form-group">
                                                                <input type="number" class="form-control" placeholder="{{translate('Min QTY')}}" name="wholesale_min_qty[]" min="1">
                                                            </div>
                                                        </div>
                                                        <div class="col-3">
                                                            <div class="form-group">
                                                                <input type="number" class="form-control" placeholder="{{ translate('Max QTY') }}" name="wholesale_max_qty[]" min="1">
                                                            </div>
                                                        </div>
                                                        <div class="col">
                                                            <div class="form-group">
                                                                <input type="number" step="0.01" class="form-control" placeholder="{{ translate('Price per piece') }}" name="wholesale_price[]" min="0">
                                                            </div>
                                                        </div>
                                                        <div class="col-auto">
                                                            <button type="button" class="mt-1 btn btn-icon btn-circle btn-sm btn-soft-danger" onclick="removeWholesalePriceRow(this)">
                                            ...
                                            <button type="button" class="btn btn-soft-secondary btn-sm" onclick="addWholesalePriceRow()">
                                                {{ translate('Add More') }}
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <!-- Ecomarch Product -->
                            <h5 class="mb-3 pb-3 fs-17 fw-700" style="border-bottom: 1px dashed #e4e5eb;">{{translate('Ecomarch Product')}}</h5>
                            <div class="w-100 mb-3">
                                <div class="form-group row">
                                    <label class="col-md-3 col-from-label">{{translate('Enable Ecomarch Product')}}</label>
                                    <div class="col-md-9">
                                        <label class="aiz-switch aiz-switch-success mb-0">
                                            <input type="checkbox" name="ecomarch_product" value="1" {{ $product->ecomarch_product ? 'checked' : '' }}>
                                            <span></span>
                                        </label>
                                        <small class="text-muted">{{translate('Enable this to show the product in regular All Products section')}}</small>
                                    </div>
                                </div>
                            </div>

                            <!-- Low Stock Quantity -->
                            <h5 class="mb-3 pb-3 fs-17 fw-700" style="border-bottom: 1px dashed #e4e5eb;">{{translate('Global Low Stock Warning')}}</h5>
                            <div class="w-100 mb-3">
                                <div class="form-group row">
                                    <label class="col-md-3 col-from-label">
                                        {{translate('Global Threshold Quantity')}}
                                        <small class="text-muted d-block">{{translate('Alert when total stock across all warehouses is below this number')}}</small>
                                    </label>
                                    <div class="col-md-9">
...
<script type="text/javascript">
    ...
    // Wholesale Product Functions
    function toggleWholesaleOptions() {
        var checkbox = $('input[name="wholesale_product"]');
        var wholesaleOptions = $('#wholesale-options');
        
        if (checkbox.is(':checked')) {
            wholesaleOptions.show();
        } else {
            wholesaleOptions.hide();
        }
    }

    function addWholesalePriceRow() {
        var html = '<div class="row gutters-5 wholesale-price-row">' +
                   '<div class="col-3">' +
                       '<div class="form-group">' +
                           '<input type="number" name="wholesale_min_qty[]" class="form-control" placeholder="{{ translate("Min QTY") }}" min="1">' +
                       '</div>' +
                   '</div>' +
                   '<div class="col-3">' +
                       '<div class="form-group">' +
                           '<input type="number" name="wholesale_max_qty[]" class="form-control" placeholder="{{ translate("Max QTY") }}" min="1">' +
                       '</div>' +
                   '</div>' +
                   '<div class="col">' +
                       '<div class="form-group">' +
                           '<input type="number" step="0.01" name="wholesale_price[]" class="form-control" placeholder="{{ translate("Price per piece") }}" min="0">' +
                       '</div>' +
                   '</div>' +
                   '<div class="col-auto">' +
                       '<button type="button" class="mt-1 btn btn-icon btn-circle btn-sm btn-soft-danger" onclick="removeWholesalePriceRow(this)">' +
                   ...
                   '</div>' +
               '</div>';
        $('.wholesale-price-section').append(html);
    }

    function removeWholesalePriceRow(button) {
        $(button).closest('.wholesale-price-row').remove();
    }
    ...
    function calculateTotalStock() {
        ...
        $('#total-stock-input').val(total);
        return total;
    }

    function validateWarehouseStocks() {
        let isValid = true;
        $('.warehouse-stock-input').each(function() {
            let value = parseInt($(this).val()) || 0;
            if (value < 0) {
    ...
    $(document).ready(function() {
        // Calculate total stock when warehouse stock inputs change
        $(document).on('input change', '.warehouse-stock-input', function() {
            validateWarehouseStocks();
            calculateTotalStock();
        });
        ...
        $('form').on('submit', function(e) {
            if (!validateWarehouseStocks()) {
                e.preventDefault();
                alert('Please enter valid stock quantities (cannot be negative)');
                return false;
            }
        });
    });
...
<script>
    $(document).ready(function(){
        ...
        // Initialize commission percentage visibility
        toggleCommissionPercentage();
    });
    
    // Toggle commission percentage field
    function toggleCommissionPercentage() {
        var enableCommission = $('input[name="enable_sales_commission"]').is(':checked');
        var commissionRow = $('#commission-percentage-row');
        var commissionInput = $('input[name="sales_commission_percentage"]');
    ...
    $(document).ready(function() {
        $('#enable_sales_commission').on('change', function() {
            if ($(this).is(':checked')) {
                $('#commission_percentage_row').fadeIn();
                $('input[name="sales_commission_percentage"]').attr('required', true);
            } else {
                $('#commission_percentage_row').fadeOut();
                $('input[name="sales_commission_percentage"]').removeAttr('required');
                $('input[name="sales_commission_percentage"]').val(0);
            }
        });
        
        // Initialize commission toggle on page load
        if ($('#enable_sales_commission').is(':checked')) {
            $('#commission_percentage_row').show();
            $('input[name="sales_commission_percentage"]').attr('required', true);
        }
    });
</script>
