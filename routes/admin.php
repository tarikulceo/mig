...
// Test routes for Sales Rep - Remove these after testing
Route::get('/test-sales-rep-login', [TestSalesRepController::class, 'testLogin'])->middleware(['auth']);
Route::get('/test-sales-rep-dashboard', [TestSalesRepController::class, 'testDashboardAccess'])->middleware(['auth', 'admin']);

// Personal Sales Routes (for Sales Representatives only)
Route::group(['prefix' => 'admin', 'middleware' => ['auth', 'admin', 'prevent-back-history']], function () {
    Route::controller(PersonalSalesController::class)->group(function () {
        Route::get('/my-sales/dashboard', 'dashboard')->name('personal.dashboard');
        Route::get('/my-sales/profile', 'profile')->name('personal.profile');
        Route::get('/my-sales/commissions', 'commissions')->name('personal.commissions');
        Route::get('/my-sales/targets', 'targets')->name('personal.targets');
        Route::get('/my-sales/activities', 'activities')->name('personal.activities');
        Route::get('/my-sales/customers', 'customers')->name('personal.customers');
    });
});
...
Route::group(['prefix' => 'admin', 'middleware' => ['auth', 'admin', 'prevent-back-history']], function () {
    ...
    Route::controller(WarehouseController::class)->group(function () {
        ...
        Route::get('/warehouse/transfer/form', 'showTransferForm')->name('warehouse.transfer.form');
        Route::get('/warehouse/transfers/create', 'showTransferForm')->name('warehouse.transfers.create');
        Route::post('/warehouse/transfer', 'transferStock')->name('warehouse.transfer');
        Route::get('/warehouse/transfers', 'transfers')->name('warehouse.transfers.index');
        Route::put('/warehouse/transfers/{transfer}/approve', 'approveTransfer')->name('warehouse.transfer.approve');
        Route::put('/warehouse/transfers/{transfer}/complete', 'completeTransfer')->name('warehouse.transfer.complete');
        Route::put('/warehouse/transfers/{transfer}/cancel', 'cancelTransfer')->name('warehouse.transfer.cancel');
        Route::get('/warehouse/alerts', 'lowStockAlerts')->name('warehouse.alerts');
        Route::put('/warehouse/alerts/{alert}/resolve', 'resolveAlert')->name('warehouse.alert.resolve');
        Route::put('/warehouse/alerts/{alert}/ignore', 'ignoreAlert')->name('warehouse.alert.ignore');
    ...
    Route::get('/warehouses/{warehouse}/reports', 'reports')->name('warehouses.reports');
        Route::get('/warehouse/product/stock', 'getProductStock')->name('warehouse.product.stock');
        Route::get('/warehouse/stock/check', 'getWarehouseStock')->name('warehouse.stock.check');
    ...
    Route::controller(SalesRepresentativeController::class)->group(function () {
        ...
        Route::put('/sales-representatives/{id}', 'update')->name('sales_representatives.update');
        Route::delete('/sales-representatives/{id}', 'destroy')->name('sales_representatives.destroy');
        
        // Product Commission Management
        Route::get('/sales-representatives/{id}/product-commissions', 'productCommissions')->name('sales_representatives.product_commissions');
    ...
    Route::controller(RetailStoreController::class)->group(function () {
        Route::get('/retail-stores', 'index')->name('retail_stores.index');
        Route::get('/retail-stores/create', 'create')->name('retail_stores.create');
        Route::get('/retail-stores/test', 'test')->name('retail_stores.test');
        Route::post('/retail-stores', 'store')->name('retail_stores.store');
        Route::get('/retail-stores/{id}', 'show')->name('retail_stores.show');
        Route::get('/retail-stores/{id}/edit', 'edit')->name('retail_stores.edit');
        Route::put('/retail-stores/{id}', 'update')->name('retail_stores.update');
        Route::delete('/retail-stores/{id}', 'destroy')->name('retail_stores.destroy');
        Route::post('/retail-stores/get-coordinates', 'getCoordinates')->name('retail_stores.get_coordinates');
    });

    // Store Visits Management
    Route::resource('store-visits', StoreVisitController::class)->names([
        'index' => 'store_visits.index',
        'create' => 'store_visits.create',
        'store' => 'store_visits.store',
        'show' => 'store_visits.show',
    ...
    // Sales Territories
    Route::controller(SalesTerritoryController::class)->group(function () {
        Route::get('/sales-territories', 'index')->name('sales_territories.index');
        Route::get('/sales-territories/create', 'create')->name('sales_territories.create');
        Route::post('/sales-territories', 'store')->name('sales_territories.store');
        Route::get('/sales-territories/{id}', 'show')->name('sales_territories.show');
        Route::get('/sales-territories/{id}/edit', 'edit')->name('sales_territories.edit');
        Route::put('/sales-territories/{id}', 'update')->name('sales_territories.update');
        Route::delete('/sales-territories/{id}', 'destroy')->name('sales_territories.destroy');
        Route::get('/get-states-by-country/{countryId}', 'getStatesByCountry')->name('get_states_by_country');
    });

    // Sales Commissions
    Route::controller(SalesCommissionController::class)->group(function () {
        Route::get('/sales-commissions', 'index')->name('sales_commissions.index');
        Route::get('/sales-commissions/create', 'create')->name('sales_commissions.create');
        Route::post('/sales-commissions', 'store')->name('sales_commissions.store');
        Route::get('/sales-commissions/{id}', 'show')->name('sales_commissions.show');
        Route::post('/sales-commissions/{id}/pay', 'pay')->name('sales_commissions.pay');
        Route::post('/sales-commissions/bulk-pay', 'bulkPay')->name('sales_commissions.bulk_pay');
        Route::get('/sales-commissions/reports', 'reports')->name('sales_commissions.reports');
    });

    // Sales Targets
    Route::controller(SalesTargetController::class)->group(function () {
        Route::get('/sales-targets', 'index')->name('sales_targets.index');
        Route::get('/sales-targets/create', 'create')->name('sales_targets.create');
        Route::post('/sales-targets', 'store')->name('sales_targets.store');
        Route::get('/sales-targets/{id}', 'show')->name('sales_targets.show');
        Route::get('/sales-targets/{id}/edit', 'edit')->name('sales_targets.edit');
        Route::put('/sales-targets/{id}', 'update')->name('sales_targets.update');
        Route::delete('/sales-targets/{id}', 'destroy')->name('sales_targets.destroy');
        Route::post('/sales-targets/update-achievement', 'updateAchievement')->name('sales_targets.update_achievement');
        Route::post('/sales-targets/bulk-create', 'bulkCreate')->name('sales_targets.bulk_create');
    });

    // Sales Activities
    Route::controller(SalesActivityController::class)->group(function () {
        Route::get('/sales-activities', 'index')->name('sales_activities.index');
        Route::get('/sales-activities/create', 'create')->name('sales_activities.create');
        Route::post('/sales-activities', 'store')->name('sales_activities.store');
        Route::get('/sales-activities/{id}', 'show')->name('sales_activities.show');
        Route::get('/sales-activities/{id}/edit', 'edit')->name('sales_activities.edit');
        Route::put('/sales-activities/{id}', 'update')->name('sales_activities.update');
        Route::delete('/sales-activities/{id}', 'destroy')->name('sales_activities.destroy');
        Route::get('/sales-activities-calendar', 'calendar')->name('sales_activities.calendar');
        Route::get('/sales-activities-calendar-data', 'getCalendarActivities')->name('sales_activities.calendar_data');
        Route::get('/sales-activities/overdue', 'overdue')->name('sales_activities.overdue');
        Route::get('/sales-activities/upcoming', 'upcoming')->name('sales_activities.upcoming');
    });
