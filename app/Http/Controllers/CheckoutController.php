...
class CheckoutController extends Controller
{
    ...
    public function checkout_done($combined_order_id, $payment)
    {
        ...
        foreach ($combined_order->orders as $key => $order) {
            ...
            // Assign warehouse to order after payment confirmation
            try {
                $warehouseService->assignOrderToWarehouse($order);
            } catch (\Exception $e) {
                // Log the error but don't fail the order
                \Log::error('Warehouse assignment failed for order ' . $order->id . ': ' . $e->getMessage());
            }
